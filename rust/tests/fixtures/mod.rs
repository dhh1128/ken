...
fn compare(name: &str) {
    ...
    println!("{:?} -> {:?}", tokens, ken::tokenize(&tokens));

    difference::assert_diff(&comp, &ken::file_to_html(md).unwrap(), " ", 0);
}
...
fn roundtrip(name: &str) {
    ...
    let v = ken::tokenize(&tokens);
    println!("{:?}", v);
    let out = ken::generate(v);

    println!("BEGIN\n{}\nEND", out);

    difference::assert_diff(&comp, &ken::to_html(&out), " ", 0);
}
